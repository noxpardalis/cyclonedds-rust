...
impl<'d, 'p, 'a, A> WaitSet<'d, 'p, 'a, A> {
    ...
    ///
    pub fn wait(&mut self) -> Result<Vec<&'a A>> {
        self.wait_for(crate::Duration::INFINITE)
    }

    ///
    pub fn wait_for(&mut self, timeout: crate::Duration) -> Result<Vec<&'a A>> {
        let (_, attachments) =
...
mod tests {
    ...
    fn test_waitset_wait() {
        ...
        let actual = waitset
            .wait_for(crate::Duration::from_nanos(5_000_000))
            .unwrap();
        ...
        let actual = waitset
            .wait_for(crate::Duration::from_nanos(1_000_000_000))
            .unwrap();
    ...
    fn test_waitset_wait_with_invalid_waitset() {
        ...
        let waitset_id = waitset.inner;
        waitset.inner = 0;
        let result = waitset.wait().unwrap_err();
        assert_eq!(result, crate::Error::BadParameter);
    ...
    fn test_waitset_wait_dynamic_data() {
        ...
        let attachments = waitset.wait().unwrap();

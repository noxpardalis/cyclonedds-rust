...
pub struct Listener<T> {
    ...
    subscriber: SubscriberListener<T>,
    publisher: PublisherListener<T>,
}
...
///
#[derive(Debug)]
pub struct SubscriberListener<T> {
...
///
#[derive(Debug)]
pub struct PublisherListener<T> {
    ///
...
impl<T> Default for SubscriberListener<T> {
    fn default() -> Self {
        Self {
            data_on_readers: Default::default(),
            reader: Default::default(),
        }
    }
}

impl<T> Default for PublisherListener<T> {
    fn default() -> Self {
        Self {
    ...
    T: serde::ser::Serialize + serde::de::DeserializeOwned + std::clone::Clone + Default,
{
    ...
    pub fn with_subscriber(
        mut self,
        setter: fn(SubscriberListener<T>) -> SubscriberListener<T>,
    ) -> Self {
    ...
    pub fn with_publisher(
        mut self,
        setter: fn(PublisherListener<T>) -> PublisherListener<T>,
    ) -> Self {
...
impl<T> AsRef<SubscriberListener<T>> for SubscriberListener<T> {
    fn as_ref(&self) -> &SubscriberListener<T> {
        self
    }
}
impl<T> AsRef<PublisherListener<T>> for PublisherListener<T> {
    fn as_ref(&self) -> &PublisherListener<T> {
        self
    }
}
...
impl<T> AsRef<Listener<T>> for Listener<T> {
    fn as_ref(&self) -> &Listener<T> {
        self
    }
}
...
impl<T> AsRef<SubscriberListener<T>> for Listener<T> {
    fn as_ref(&self) -> &SubscriberListener<T> {
        &self.subscriber
    }
}
impl<T> AsRef<PublisherListener<T>> for Listener<T> {
    fn as_ref(&self) -> &PublisherListener<T> {
        &self.publisher
    }
}
...
mod tests {
    ...
    fn receive_listener<L, T>(listener: L)
    where
        L: AsRef<Listener<T>>,
    {
    ...
    fn receive_subscriber_listener<L, T>(listener: L)
    where
        L: AsRef<SubscriberListener<T>>,
    {
        let _ = listener.as_ref();
        assert!(true);
    }

    fn receive_publisher_listener<L, T>(listener: L)
    where
        L: AsRef<PublisherListener<T>>,
    {
        let _ = listener.as_ref();
        assert!(true);
    }
    ...
    fn test_listener_create() {
        let listener = Listener::<crate::tests::topic::Data>::new()
            .with_topic(|topic| topic.with_inconsistent_topic(|_, _| unreachable!()))
            ...
            .with_publisher(|publisher| {
                publisher.with_writer(|writer| {
                    writer
                        .with_liveliness_lost(|_, _| unreachable!())
                        .with_offered_deadline_missed(|_, _| unreachable!())
                        .with_offered_incompatible_qos(|_, _| unreachable!())
                        .with_publication_matched(|_, _| unreachable!())
                })
            });
        let topic_listener = TopicListener::<crate::tests::topic::Data>::new()
            .with_inconsistent_topic(|_, _| unreachable!());
        let subscriber_listener = SubscriberListener::<crate::tests::topic::Data>::new()
            .with_data_on_readers(|_| unreachable!())
            .with_reader(|reader| {
                reader
                    .with_data_available(|_| unreachable!())
        ...
        let publisher_listener =
            PublisherListener::<crate::tests::topic::Data>::new().with_writer(|writer| {
                writer
                    .with_liveliness_lost(|_, _| unreachable!())
                    .with_offered_deadline_missed(|_, _| unreachable!())
                    .with_offered_incompatible_qos(|_, _| unreachable!())

...
pub unsafe extern "C" fn from_ser<T>(
    _serdata: *const cyclonedds_sys::ddsi_sertype,
    _: u32,
    _: *const cyclonedds_sys::ddsi_rdata,
    _: usize,
...
pub unsafe extern "C" fn from_keyhash<T>(
    ...
    _: *const cyclonedds_sys::ddsi_keyhash,
) -> *mut cyclonedds_sys::ddsi_serdata {
    todo!()
}
    ...
    T: std::clone::Clone + serde::ser::Serialize,
{
    ...
    if serdata.serialized_sample.is_none() {
        if let Some(sample) = serdata.sample() {
            serdata.serialized_sample =
                cdr_encoding::to_vec::<_, byteorder::LittleEndian>(sample).ok();
        }
    }
    ...
    T: std::clone::Clone,
{
    ...
    if let Some(data) = serdata.sample() {
        ...
        // FIXME this results in either a memory leak (with) or access to uninitialized memory
        // (without).
...
pub unsafe extern "C" fn get_keyhash<T>(
    _serdata: *const cyclonedds_sys::ddsi_serdata,
    _: *mut cyclonedds_sys::ddsi_keyhash,
    _: bool,
) {
    todo!();
}
...
pub unsafe extern "C" fn from_loaned_sample<T>(
    ...
    _: u32,
    _: *const i8,
    _: *mut cyclonedds_sys::dds_loaned_sample,
    _: bool,
) -> *mut cyclonedds_sys::ddsi_serdata {
    todo!()
}
...
pub unsafe extern "C" fn from_psmx<T>(
    _sertype: *const cyclonedds_sys::ddsi_sertype,
    _loan: *mut cyclonedds_sys::dds_loaned_sample,
) -> *mut cyclonedds_sys::ddsi_serdata {
    todo!()
}
